...
interface CallContextType {
  ...
  startCall: (conversationId: string, participantIds: string[]) => void;
...
export const CallProvider = ({ children, currentUser }: CallProviderProps) => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // If an incoming call is updated to active, clear it
          ...
          if (updatedCall.status === 'ended' || updatedCall.status === 'declined') {
            ...
            // If the caller's call becomes active, set it as active and clear incoming
            setActiveCall(updatedCall);
            setIncomingCall(null);
          }
        }
  ...
  const startCall = useCallback(async (conversationId: string, participantIds: string[]) => {
    ...
    // Check for existing active or ringing calls in this conversation
    const { data: existingCalls, error: existingCallsError } = await supabase
      .from('calls')
      .select('id, status')
    ...
    if (existingCallsError) {
      console.error("[CallProvider] Error checking existing calls:", existingCallsError);
    ...
    // Generate a unique Jitsi Meet URL
    const callUrl = `https://meet.jit.si/${`Snipchat-${conversationId}-${Date.now()}`}`;
  ...
  const acceptCall = useCallback(async (call: Call) => {
    if (!currentUser) return;
    const { error } = await supabase.from('calls').update({ status: 'active' }).eq('id', call.id).eq('conversation_id', call.conversation_id);
    if (error) {
  ...
  const declineCall = useCallback(async (call: Call) => {
    if (!currentUser) return;
    const { error } = await supabase.from('calls').update({ status: 'declined' }).eq('id', call.id).eq('conversation_id', call.conversation_id);
    if (error) {
  ...
  const endCall = useCallback(async (call: Call) => {
    if (!currentUser) return;
    const { error } = await supabase.from('calls').update({ status: 'ended' }).eq('id', call.id).eq('conversation_id', call.conversation_id);
    if (error) {
